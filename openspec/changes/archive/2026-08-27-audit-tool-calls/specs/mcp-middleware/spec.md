...
- **WHEN** a tool is executed via MCP `execute` or `POST /tools/{key}`, and its underlying callable is actually invoked
- **THEN** the resolved username, tool key, and parameters are logged, regardless of which transport the call arrived through
...
When a call exceeds the resolved limit: on the MCP path, `execute` SHALL NOT be invoked, the handler SHALL log the rejection (tool key, resolved username, and that it was throttled) before returning, and a plain-text tool result (no error flag) SHALL be returned stating which tool was throttled and the seconds remaining until the window resets. On the `POST /tools/{key}` path, the tool SHALL NOT be invoked, the handler SHALL log the rejection the same way, and the HTTP response SHALL indicate the call was throttled.
...
- **THEN** an error is raised at process startup, before any request is handled
...
- **THEN** the rejecting handler logs the call explicitly, and FastMCP's `TimingMiddleware` still records its timing on the MCP path
...
The server SHALL record `mcc_tool_calls_total{tool, status}` (a counter) and `mcc_tool_call_duration_seconds{tool}` (a histogram) for every catalog tool call for which `ToolModel.call()` actually invokes the tool's underlying callable, via the MCP `execute` tool or the `POST /tools/{key}` HTTP route, using the same hook fired from within `ToolModel.call()` for both transports, labeled by the exact catalog tool key. A call rejected by the rate-limit check or served from `execute()`'s result cache SHALL NOT be recorded, since neither reaches `ToolModel.call()`.

#### Scenario: MCP execute call recorded
- **WHEN** a catalog tool is called via the MCP `execute` tool and its underlying callable is invoked
...
- **WHEN** a catalog tool is called via `POST /tools/{key}` and its underlying callable is invoked
...
#### Scenario: Throttled call not recorded
- **WHEN** a call is rejected by the rate-limit check, via either transport
- **THEN** `mcc_tool_calls_total`/`mcc_tool_call_duration_seconds` are not incremented for that call

#### Scenario: Cache hit not recorded
- **WHEN** an `execute()` call is served entirely from its `cache_ttl` cached result
- **THEN** `mcc_tool_calls_total`/`mcc_tool_call_duration_seconds` are not incremented for that call
