...
class TestBase64:
    async def test_encode(self):
        assert (
            await execute("public.text.base64_encode", {"text": "hello"}) == "aGVsbG8="
        )

    async def test_decode(self):
        assert (
            await execute("public.text.base64_decode", {"text": "aGVsbG8="}) == "hello"
        )

    async def test_roundtrip(self):
        encoded = await execute("public.text.base64_encode", {"text": "round trip 🚀"})
        decoded = await execute("public.text.base64_decode", {"text": encoded})
...
class TestHash:
    async def test_sha256_default(self):
        ...
        assert await execute("public.text.hash", {"text": "hello"}) == expected

    async def test_md5(self):
        expected = hashlib.md5(b"hello").hexdigest()
        assert (
            await execute("public.text.hash", {"text": "hello", "algo": "md5"})
            == expected
        )
...
class TestDiff:
    async def test_identical(self):
        result = await execute("public.text.diff", {"a": "same", "b": "same"})
        assert result == ""
