...
class TestCreateAndList:
    async def test_create_zip(self, tmp_path):
        ...
        result = await execute("admin.archive.create", {"dest": dest, "files": files})
    ...
    async def test_list_zip(self, tmp_path):
        files = _make_files(tmp_path)
        dest = str(tmp_path / "out.zip")
        await execute("admin.archive.create", {"dest": dest, "files": files})
        result = await execute("admin.archive.list", {"path": dest})
...
class TestExtract:
    async def test_extract_zip(self, tmp_path):
        ...
        await execute("admin.archive.create", {"dest": archive, "files": files})
        dest = str(tmp_path / "extracted")
        result = await execute("admin.archive.extract", {"path": archive, "dest": dest})
        assert sorted(result) == ["a.txt", "b.txt"]
    ...
    async def test_creates_dest_dir(self, tmp_path):
        ...
        await execute("admin.archive.extract", {"path": archive, "dest": dest})
